...
export default function ChatInterface({ user, onBack, onToggleFavorite, onViewProfile, isFavorite, formData }: ChatInterfaceProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black flex flex-col">
      ...
      <div className="fixed inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-slate-900/15 via-transparent to-transparent"></div>
      <div className="fixed inset-0 bg-[linear-gradient(45deg,transparent_25%,rgba(100,116,139,0.03)_50%,transparent_75%)]"></div>
      
      {/* Chat Header - Your brand style */}
      <div className="sticky top-0 bg-black/70 backdrop-blur-lg border-b border-slate-500/20 p-4 z-20">
        <div className="flex items-center justify-between max-w-4xl mx-auto">
          <div className="flex items-center space-x-4">
            <button
              onClick={onBack}
              className="p-2 text-gray-400 hover:text-white transition-colors duration-300 rounded-full hover:bg-slate-500/10"
            >
            ...
            <div>
              <div className="flex items-center space-x-2">
                <button 
                  ...
                  className="text-lg font-semibold text-white hover:text-slate-300 transition-colors cursor-pointer"
                >
                  {user.username}
                </button>
          ...
          <button 
            ...
            className={`relative p-3 rounded-full transition-all duration-300 group ${
              isFavorite 
                ? 'bg-pink-900 text-pink-300 shadow-sm scale-110' 
                : 'bg-glass-medium border border-slate-500/30 text-slate-400 hover:bg-slate-500/20 hover:scale-105 focus:bg-slate-500/20 focus:scale-105 focus:outline-none focus:ring-2 focus:ring-slate-500/50'
            }`}
          >
            ...
            {!isFavorite && (
              <div className="absolute -top-1 -right-1 w-5 h-5 bg-pink-800 rounded-full flex items-center justify-center shadow-md group-hover:bg-pink-700 group-focus:bg-pink-700 transition-all duration-300 group-hover:scale-110 group-focus:scale-110">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {['Hey there! 😉', '👋 Hi!', 'What\'s up? 💕'].map((suggestion) => (
                  <button
                    ...
                    className="px-4 py-2 bg-glass-medium border border-slate-500/30 rounded-xl text-slate-400 hover:text-slate-300 hover:border-slate-400 hover:bg-glass-dark transition-all duration-300 text-sm"
                  >
                    {suggestion}
                  </button>
                ))}
                ...
                return (
                  ...
                  >
                    <div className={`relative max-w-xs md:max-w-md px-4 py-3 rounded-2xl shadow-sm ${
                      isOwn
                        ? 'bg-gradient-to-r from-slate-600 to-slate-700 text-white border border-slate-500/30'
                        : 'bg-glass-medium border border-gray-600 text-white'
                    }`}>
                      {/* WhatsApp-like message tail */}
                      <div className={`absolute top-2 w-0 h-0 ${
                        isOwn 
                          ? '-right-2 border-l-8 border-l-slate-600 border-t-8 border-t-transparent'
      ...
      <div className="sticky bottom-0 bg-black/70 backdrop-blur-lg border-t border-slate-500/20 p-4 z-20">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              className="flex-shrink-0 w-12 h-12 flex items-center justify-center rounded-full bg-glass-medium border border-gray-600 text-gray-400 hover:text-slate-300 hover:border-slate-400 transition-all duration-300"
            >
              <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clipRule="evenodd" />
              </svg>
            ...
            <div className="flex-1">
              <input
                ...
                className="w-full h-12 bg-glass-medium backdrop-blur-lg border border-gray-600/50 rounded-full px-4 text-white placeholder-gray-400 focus:border-slate-400 focus:ring-0 focus:outline-none transition-colors duration-300 text-sm"
              />
            </div>
            ...
            {inputText.trim() ? (
              <button
                ...
                className="flex-shrink-0 w-12 h-12 flex items-center justify-center rounded-full bg-gradient-to-r from-slate-600 to-slate-700 text-white hover:shadow-glow hover:scale-105 transition-all duration-300"
              >
              ...
              <button
                ...
                className={`flex-shrink-0 w-12 h-12 flex items-center justify-center rounded-full transition-all duration-300 text-white ${
                  isRecording
                    ...
                    : 'bg-gradient-to-r from-slate-600 to-slate-700 hover:from-slate-700 hover:to-slate-800 hover:scale-105 shadow-md'
                }`}
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
      ...
      <AnimatePresence>
        {showFavPopup && (
          ...
          >
            <div className="bg-black/90 backdrop-blur-xl border border-slate-500/20 rounded-2xl px-8 py-6 shadow-2xl max-w-xs w-full mx-auto">
              <div className="text-center">
                <div className="w-12 h-12 bg-gradient-to-r from-pink-700 to-pink-800 rounded-full flex items-center justify-center mx-auto mb-4">
                  <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 20 20">
